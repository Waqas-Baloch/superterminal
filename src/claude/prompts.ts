...
2. Implement the task with the smallest correct change set. Use str_replace for existing files (choose a unique snippet; expand it if the tool reports duplicates) and write_file only for new files.
3. Match the project's existing style, naming, and patterns. Do not add dependencies unless the task is impossible without them. Do not refactor, rename, or clean up beyond the task.
